...
(function (_qc) {
    ...
    _qc.Object.extend(FLAProxy.prototype, {
        prefix: '_TC_QC_flaProxy_', init: function () {
            var _me = this, conId = function () {
                ...
                _qc.Console.info("���򴰿ڱ�ʶ�� __qc_wId : " + ret);
                return ret;
            ...
            if (appId > 0) {
                setTimeout(function () {
                    ...
                    if (aToken.openid && aToken.accessToken) {
                        ...
                        _qc.Console.log("openid��accessToken��ʧ������������ջ : [" + api + "]��ջ��С��" + _pendingPool.length);
                    }
                }, 10);
            } else {
                _pendingPool.push(req);
                _qc.Console.log((_proxy.isReady && appId < 0 ? "token��ȡʧ�ܣ�������û���¼����" : "api������δ��ʼ���ɹ�") + "������������ջ : [" + api + "]��ջ��С��" + _pendingPool.length);
            }
    ...
    var Login = function () {
                    ...
                    if (!opts.showModal) {
                        ...
                        btn.firstChild.onclick = function(){
                            ...
                            if (!iframe.src) {
                                ...
                                params += "response_type=token&" // ��ȡ��access_token
                                params += "client_id=" + appid +"&" // Ӧ�õ�appid
                                params += "redirect_uri=" + opts.redirectURI // ��¼�ɹ�����ת�Ĵ���ҳ��
        ...
        _insertButton._getPopupUrl = function (opts) {
            var scope = opts['scope'] || 'AllContent.vue', redirectURI = opts['redirectURI'] || '',
            ...
            return access_token || function () {
                ...
                if ((__qc__k = $Cookie.get("__qc__k"))) {
                    ...
                    if (__qc__k.length && _keys.length != 2) {
                        throw new Error("QQConnect -> cookie : __qc__k ��ʽ�Ƿ�");
                    }
                }
        ...
        var retFun = function (opts, loginFun, logoutFun, __outCallFlag) {
            var args = arguments;
            var result = null;
            // ������Login��ʼ��ʱ����access_token 2018-8-16 zhiyonglao
    ...
    (function () {
        ...
        if (aid) {
            _qc.Console.info("��⵽�Զ���ʼ������\nappId:" + aid + "\nrUri:" + rUri);
            if (isNaN(aid)) {
                _qc.Console.error("����appid����");
            } else if (rUri && rUri.indexOf("http") != 0) {
                _qc.Console.error("����rediectURI����");
            } else {
