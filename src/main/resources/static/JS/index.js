...
function abc () {
    var url="http://localhost:8080/orange/index"
    $.ajax({
        type: "get",
        url: url,
//      data: "para="+para,  此处data可以为 a=1&b=2类型的字符串 或 json数据。
        ...
        success: function (data ,textStatus, jqXHR)
        {
            ...
            if(data.status=="1"){
                n=0;
                var  aphoto; var  name; var title;var content; var user_photo;
                boxs=data.data;
                getJsonLength(boxs);
                console.log(n);
                var str;
...
function  setPosition(){
    ...
    //重置父级的样式,这样保证图片整体居中
    oParent.style.cssText="width:" + num*iPinW + "px;margin:20px auto;";
    ...
    for (var i = 0; i<aPin.length; i++) {
        if(i<num){
            //成行元素
            compareArray[i] = aPin[i].offsetHeight+525;
        }else{
            //获取成行元素中高度最低的值
